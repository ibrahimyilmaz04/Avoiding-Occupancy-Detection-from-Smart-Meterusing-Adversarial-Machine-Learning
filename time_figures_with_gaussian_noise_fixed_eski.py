...
def test(epsilon):
    ...
    perturbed_data_full = torch.empty(0, 30, 5)
    perturbed_data_full = perturbed_data_full.to(device)
    x_test_full = torch.empty(0, 30, 5)
...
consumption_d = pd.DataFrame({
   'consump': readings_short_equal['consump'].tolist(),
   'consump_perturbed': readings_short_equal['consump_perturbed'].tolist()
...
consumption_cumul_d = pd.DataFrame({
   'consump_cumul': readings_short_equal['consump_cumul'].tolist(),
   'consump_pert_cumul': readings_short_equal['consump_pert_cumul'].tolist()
   }, index=readings_short_equal['time_stamp'].tolist())
