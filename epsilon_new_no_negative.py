...
def test(epsilon):
    ...
    i = 0
    flag = True
    perturbed_data_full = torch.empty(0)
    perturbed_data_full = perturbed_data_full.to(device)
    x_test_full = torch.empty(0)
    x_test_full = x_test_full.to(device)
    
    global perturbation_list
    for batch_idx, (X_batch, y_batch) in enumerate(test_loader):
        ...
        if flag == True:
            ...
            perturbation_list = perturbation_list + pertubation.tolist()
            if batch_idx == 0 :
                perturbed_data = torch.flatten(x_test_var)
            else:
                perturbed_data = torch.flatten(x_test_var) + torch.flatten(pertubation)
                flag = False
        else:
            perturbed_data = torch.flatten(x_test_var) - torch.flatten(pertubation)
            # perturbation_list = perturbation_list + (-pertubation).tolist()
            flag = True
        # print(flag, batch_idx)
        
        perturbed_data_full = torch.cat([perturbed_data_full, perturbed_data], dim=0)
        x_test_full = torch.cat([x_test_full, torch.flatten(x_test_var)], dim=0)

    return perturbed_data_full, x_test_full

eps = 0.05
perturbed_data_full, x_test_full = test(eps)

print(perturbed_data_full.shape)
# print(perturbed_data_full.shape[0])
print(x_test_full.shape)

# print(len(perturbation_list))
# print(len(perturbation_list[0]))
# print(len(perturbation_list[0][0]))
# for i in perturbation_list[0]:
    # print(i)
...
# readings = pd.DataFrame(torch.flatten(perturbed_data_full).tolist(), columns=['consump_perturbed'])
readings = pd.DataFrame(perturbed_data_full.tolist(), columns=['consump_perturbed'])
# readings['consump'] = torch.flatten(x_test_full).tolist()
readings['consump'] = x_test_full.tolist()
...
# readings_short = readings.iloc[:int(len(readings)/83), :]
readings_short = readings.iloc[:108000, :]
...
readings_short_equal['consump_pert_cumul'] = readings_short_equal['consump_perturbed'].cumsum()

readings_short_equal.loc[readings_short_equal['consump_perturbed'] < 0, 'consump_perturbed'] = 0
...
consumption_d = pd.DataFrame({
   'original electricity consumption': readings_short_equal['consump'].tolist(),
   'perturbed electricity consumption': readings_short_equal['consump_perturbed'].tolist()
   }, index=readings_short_equal['time_stamp'].tolist())
 
plt.figure(1, figsize=(8,8))
consumption_d.iloc[start_time:end_time,:].plot.line(figsize=(10,5))
plt.xlabel('Time')
...
consumption_cumul_d = pd.DataFrame({
   'original total electricity consumption': readings_short_equal['consump_cumul'].tolist(),
   'perturbed total electricity consumption': readings_short_equal['consump_pert_cumul'].tolist()
   }, index=readings_short_equal['time_stamp'].tolist())

plt.figure(2, figsize=(8,8))
# consumption_cumul_d.iloc[start_time:end_time,:].plot.line(figsize=(10,5))
consumption_cumul_d.iloc[:,:].plot.line(figsize=(10,5))
plt.xlabel('Time')
...
print(consumption_d.iloc[start_time:end_time,:])
print()
print(consumption_cumul_d.iloc[start_time:end_time,:])
