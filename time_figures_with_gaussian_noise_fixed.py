...
def test():
    ...
    for batch_idx, (X_batch, y_batch) in enumerate(test_loader):
        ...
        if add_noise:
            if batch_idx==0:
                noise = np.random.normal(mu, sigma, x_test_var.shape)
                noise = torch.from_numpy(noise).float().to(device)
            if x_test_var.shape != noise.shape:
                noise = np.random.normal(mu, sigma, x_test_var.shape)
                noise = torch.from_numpy(noise).float().to(device)
            perturbation_list = perturbation_list + noise.tolist()
            x_test_perturbed = noise + x_test_var
        perturbed_data_full = torch.cat([perturbed_data_full, x_test_perturbed], dim=0)
        x_test_full = torch.cat([x_test_full, x_test_var], dim=0)
    return perturbed_data_full, x_test_full

perturbed_data_full, x_test_full = test()
...
plt.title('electricity consumption over time for home ' + str(home_number) + ' (' + season_name + ')')
